...
def arm(cli: Path, daemon: Path, repo: Path, name: str, timeout: int, experiment: str, check_scenario: str) -> dict[str, Any]:
    ...
    try:
        ...
        if name in {"ExplicitClippyControl", "ExplicitClippyShadow"}:
...
def main() -> int:
    ...
    treatment = {"passthrough": "Passthrough", "identity": "Identity", "path": "Path", "explicit-aa": "ExplicitB", "explicit-shadow": "ExplicitShadow", "explicit-active": "ExplicitActive", "explicit-check-shadow": "ExplicitCheckShadow", "explicit-check-active": "ExplicitCheckActive", "explicit-check-shadow-v2": "ExplicitCheckV2Shadow", "explicit-check-active-v2": "ExplicitCheckV2Active", "explicit-clippy-shadow": "ExplicitClippyShadow"}[a.hook_mode]
    control = "ExplicitA" if a.hook_mode == "explicit-aa" else ("ExplicitActiveControl" if a.hook_mode == "explicit-active" else ("ExplicitControl" if a.hook_mode == "explicit-shadow" else ("ExplicitCheckControl" if a.hook_mode == "explicit-check-shadow" else ("ExplicitCheckActiveControl" if a.hook_mode == "explicit-check-active" else ("ExplicitCheckV2Control" if a.hook_mode in {"explicit-check-shadow-v2","explicit-check-active-v2"} else ("ExplicitClippyControl" if a.hook_mode == "explicit-clippy-shadow" else "Control"))))))
    experiment = "source-active-pilot-001" if a.hook_mode == "explicit-active" else ("source-cargo-check-shadow-001" if a.hook_mode == "explicit-check-shadow" else (f"source-cargo-check-active-{a.check_scenario}-001" if a.hook_mode == "explicit-check-active" else ("source-cargo-check-v2-shadow-001" if a.hook_mode == "explicit-check-shadow-v2" else (f"source-cargo-check-v2-active-{a.check_scenario}-001" if a.hook_mode == "explicit-check-active-v2" else ("source-cargo-clippy-shadow-001" if a.hook_mode == "explicit-clippy-shadow" else EXPERIMENT)))))
    ...
    phase = "5.4" if a.hook_mode == "explicit-clippy-shadow" else ("5.3" if a.hook_mode in {"explicit-check-active","explicit-check-shadow-v2","explicit-check-active-v2"} else ("5.2" if a.hook_mode == "explicit-check-shadow" else ("5.1" if a.hook_mode == "explicit-active" else "5.0")))
