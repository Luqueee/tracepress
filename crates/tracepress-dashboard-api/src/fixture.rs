            ...
            for request_index in 0..requests_per_session {
                ...
                .enumerate()
                {
                    ...
                    let _candidate = transaction.execute(
                        ...
                        params![candidate_id, snapshot_id, block_id, compressor, status, original, candidate, if applicable { Some(32_i64) } else { None }, if request_index == 0 { "high" } else { "medium" }, if compressor == &"json.readable_table" || compressor == &"json.compact_records" || compressor == &"json.key_elision" || compressor == &"json.empty_noise_fields" || compressor == &"json.repeated_value_elision" || compressor == &"shell.diagnostic_projection" { "human_readable_structured" } else if compressor == &"json.minify" || compressor == &"json.noop" { "provider_compatible_control" } else { "opaque_custom_encoding" }],
                    )?;
