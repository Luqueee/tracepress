...
def main() -> int:
    p=argparse.ArgumentParser(description=__doc__); p.add_argument("--repo-root",type=Path,default=Path(__file__).resolve().parents[1]); p.add_argument("--workload-root",type=Path,default=Path("/tmp/tracepress-source-passthrough-aa-001")); p.add_argument("--pairs",type=int,default=1,choices=range(1,11)); p.add_argument("--timeout",type=int,default=180); p.add_argument("--hook-mode",choices=("passthrough","identity","path","explicit-aa","explicit-shadow"),default="passthrough"); p.add_argument("--output-json",type=Path,required=True); p.add_argument("--output-md",type=Path,required=True); a=p.parse_args()
    ...
    treatment = {"passthrough": "Passthrough", "identity": "Identity", "path": "Path", "explicit-aa": "ExplicitB", "explicit-shadow": "ExplicitShadow"}[a.hook_mode]
    control = "ExplicitA" if a.hook_mode == "explicit-aa" else ("ExplicitControl" if a.hook_mode == "explicit-shadow" else "Control")
