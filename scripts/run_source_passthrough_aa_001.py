...
def arm(cli: Path, daemon: Path, repo: Path, name: str, timeout: int, experiment: str, check_scenario: str, rg_scenario: str) -> dict[str, Any]:
    ...
    try:
        if name in {"ExplicitGitStatusControl", "ExplicitGitStatusShadow"}:
        ...
        if name in {"ExplicitGitStatusControl", "ExplicitGitStatusShadow"}:
            prompt=f"Your first and only Bash command must be exactly `{cli} tool git status`. Do not run any other Bash command, inspect files, or modify source. End with exactly STATUS_DIRTY if the working tree has changes or STATUS_CLEAN if it is clean, and no other text."
            passed_sentinel, failed_sentinel = "STATUS_DIRTY", "STATUS_CLEAN"
...
def main() -> int:
    ...
    phase = "5.7" if a.hook_mode == "explicit-git-status-shadow" else ("5.6" if a.hook_mode == "explicit-rg-active" else ("5.5" if a.hook_mode == "explicit-rg-shadow" else ("5.4" if a.hook_mode == "explicit-clippy-shadow" else ("5.3" if a.hook_mode in {"explicit-check-active","explicit-check-shadow-v2","explicit-check-active-v2"} else ("5.2" if a.hook_mode == "explicit-check-shadow" else ("5.1" if a.hook_mode == "explicit-active" else "5.0"))))))
