...
Fail open is tested at the forwarding seam, not by inspecting internal logs. For a valid request, inject each failure independently: optimizer failure, policy failure, auxiliary SQLite busy or write failure, CAS failure, serialization failure, telemetry queue full, OTel backend offline, and redaction metadata failure. The observable result must forward the original request with unchanged body bytes, or return a typed transport error if upstream forwarding itself cannot proceed. The auxiliary failure must never block or mutate the transparent request.

Malformed input is different from an optimization failure. Malformed transport frames, oversized frames, invalid authentication, and invalid route syntax are rejected with a typed error and non-success status. Invalid UTF-8, NUL bytes, binary bodies, deep JSON, huge output, infinite output, malicious ANSI, and decompression-bomb-shaped metadata are treated as opaque bytes or safely bypassed within finite bounds. No parser may allocate based on an unbounded claimed length.
