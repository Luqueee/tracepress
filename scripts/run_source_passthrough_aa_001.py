...
def prepare_git_status_workload(workspace: Path) -> None:
    readme = workspace / "README.md"
    readme.write_text(readme.read_text(encoding="utf-8") + "\n", encoding="utf-8")
    staged = workspace / ".tracepress-phase-5-7-staged"
    staged.write_text("public controlled fixture\n", encoding="utf-8")
    subprocess.run(["git", "-C", str(workspace), "add", staged.name], check=True, capture_output=True, text=True, timeout=30)
    (workspace / ".tracepress-phase-5-7-untracked").write_text("public controlled fixture\n", encoding="utf-8")

def arm(cli: Path, daemon: Path, repo: Path, name: str, timeout: int, experiment: str, check_scenario: str, rg_scenario: str, git_status_scenario: str) -> dict[str, Any]:
    workspace = isolated_worktree(repo)
    state = Path(tempfile.mkdtemp(prefix="tp5-aa-", dir="/tmp")); log = None; process = None
    started = time.monotonic()
    try:
        if name in {"ExplicitGitStatusControl", "ExplicitGitStatusShadow", "ExplicitGitStatusActiveControl", "ExplicitGitStatusActive"} and git_status_scenario == "dirty":
            prepare_git_status_workload(workspace)
        ...
        if name == "ExplicitGitStatusShadow": env["TRACEPRESS_SOURCE_REDUCER"] = "git_status_v1_shadow"
        if name == "ExplicitGitStatusActive": env["TRACEPRESS_SOURCE_REDUCER"] = "git_status_v1_active"
        ...
        if name in {"ExplicitGitStatusControl", "ExplicitGitStatusShadow", "ExplicitGitStatusActiveControl", "ExplicitGitStatusActive"}:
            recovery = " If and only if the output is insufficient, you may run the exact Tracepress recovery command printed in the output or rerun the same wrapper command." if name in {"ExplicitGitStatusActiveControl", "ExplicitGitStatusActive"} else ""
            prompt=f"Your first Bash command must be exactly `{cli} tool git status`.{recovery} Do not run any other Bash command, inspect files, or modify source. End with exactly STATUS_DIRTY if the working tree has changes or STATUS_CLEAN if it is clean, and no other text."
            passed_sentinel, failed_sentinel = ("STATUS_DIRTY", "STATUS_CLEAN") if git_status_scenario == "dirty" else ("STATUS_CLEAN", "STATUS_DIRTY")
        elif name in {"ExplicitRgControl", "ExplicitRgShadow", "ExplicitRgActiveControl", "ExplicitRgActive"}:
...
def main() -> int:
    ...
    phase = "5.8" if a.hook_mode == "explicit-git-status-active" else ("5.7" if a.hook_mode == "explicit-git-status-shadow" else ("5.6" if a.hook_mode == "explicit-rg-active" else ("5.5" if a.hook_mode == "explicit-rg-shadow" else ("5.4" if a.hook_mode == "explicit-clippy-shadow" else ("5.3" if a.hook_mode in {"explicit-check-active","explicit-check-shadow-v2","explicit-check-active-v2"} else ("5.2" if a.hook_mode == "explicit-check-shadow" else ("5.1" if a.hook_mode == "explicit-active" else "5.0")))))))
    scenario = a.git_status_scenario if a.hook_mode == "explicit-git-status-active" else (a.rg_scenario if a.hook_mode == "explicit-rg-active" else (a.check_scenario if a.hook_mode in {"explicit-check-active","explicit-check-shadow-v2","explicit-check-active-v2"} else None))
