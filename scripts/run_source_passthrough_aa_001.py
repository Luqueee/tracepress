...
def arm(cli: Path, daemon: Path, repo: Path, name: str, timeout: int, experiment: str, check_scenario: str) -> dict[str, Any]:
    ...
    try:
        ...
        if name in {"ExplicitRgControl", "ExplicitRgShadow"}:
            prompt=f"Your first and only Bash command must be exactly `{cli} tool rg -n struct crates`. Do not run any other Bash command, inspect files, or modify source. End with exactly SEARCH_MATCHED if the command finds matches or SEARCH_NO_MATCH if it finds none, and no other text."
...
def main() -> int:
    ...
    phase = "5.5" if a.hook_mode == "explicit-rg-shadow" else ("5.4" if a.hook_mode == "explicit-clippy-shadow" else ("5.3" if a.hook_mode in {"explicit-check-active","explicit-check-shadow-v2","explicit-check-active-v2"} else ("5.2" if a.hook_mode == "explicit-check-shadow" else ("5.1" if a.hook_mode == "explicit-active" else "5.0"))))
