...
use crate::{
    ...
    encode::{
        ...
        context_correlation_status_text, context_origin_text, context_role_text,
...
fn insert(
    ...
    match command {
        ...
        WriteCommand::ShadowCompressionExperimentCounters {
            ...
            "UPDATE compression_experiments SET forwarding_mutations = forwarding_mutations + ?2, shadow_drops = shadow_drops + ?3, shadow_jobs_admitted = shadow_jobs_admitted + ?4, shadow_jobs_processed = shadow_jobs_processed + ?5, shadow_job_drops = shadow_job_drops + ?6, candidate_evaluations_attempted = candidate_evaluations_attempted + ?7, candidate_evaluations_completed = candidate_evaluations_completed + ?8, candidate_evaluation_drops = candidate_evaluation_drops + ?9, shadow_queue_full_drops = shadow_queue_full_drops + ?10, shadow_byte_budget_drops = shadow_byte_budget_drops + ?11, shadow_work_budget_drops = shadow_work_budget_drops + ?12, shadow_worker_closed_drops = shadow_worker_closed_drops + ?13, shadow_persistence_drops = shadow_persistence_drops + ?14, recovery_failures = recovery_failures + ?15, determinism_failures = determinism_failures + ?16 WHERE experiment_id = ?1",
            params![
                experiment_id,
                sqlite_u64(*forwarding_mutations, "forwarding_mutations")?,
                sqlite_u64(*shadow_drops, "shadow_drops")?,
                sqlite_u64(*shadow_jobs_admitted, "shadow_jobs_admitted")?,
                sqlite_u64(*shadow_jobs_processed, "shadow_jobs_processed")?,
                sqlite_u64(*shadow_job_drops, "shadow_job_drops")?,
                sqlite_u64(*candidate_evaluations_attempted, "candidate_evaluations_attempted")?,
                sqlite_u64(*candidate_evaluations_completed, "candidate_evaluations_completed")?,
                sqlite_u64(*candidate_evaluation_drops, "candidate_evaluation_drops")?,
        ...
        WriteCommand::ProviderRequest {
            ...
            committed(sqlite(transaction.execute(
                ...
                transport.as_ref().map(|value| provider_transport_text(*value)),
                (*endpoint_profile_version).map(|value| sqlite_u64(u64::from(value), "endpoint_profile_version")).transpose()?,
                transport.map(|_value| "provider_reported"),
                transport.map(|value| match value {
                    crate::ProviderTransport::OpenAiPublicApi => "openai_api",
                    crate::ProviderTransport::ChatGptCodexSubscription => "unknown",
                }),
                content_encoding.as_ref().map(|value| content_encoding_text(*value)),
                analysis_decode_status.as_ref().map(|value| analysis_decode_status_text(*value)),
                (*wire_bytes).map(|value| sqlite_u64(value, "wire_bytes")).transpose()?,
                wire_sha256.as_deref(),
