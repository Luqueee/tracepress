...
const MIGRATIONS: [&str; 11] = [
    ...
    include_str!("../../tracepress-storage/migrations/0011_provider_candidate_metadata.sql"),
];
...
pub(crate) fn create(large: bool) -> Result<FixtureDatabase, rusqlite::Error> {
    ...
    if !large {
        let _experiment = transaction.execute(
            "INSERT INTO compression_experiments(experiment_id,compressor_set_json,runtime_sha,limits_json,status,started_at,completed_at,forwarding_mutations,shadow_drops,recovery_failures,determinism_failures) VALUES ('shadow-pilot-001','[[\"json.noop\",1],[\"json.minify\",1],[\"json.tabular\",1],[\"json.repeated_subtree\",1],[\"json.readable_table\",1],[\"json.compact_records\",1],[\"json.key_elision\",1],[\"json.empty_noise_fields\",1],[\"json.repeated_value_elision\",1],[\"shell.diagnostic_projection\",1],[\"search.result_projection\",1]]','93ffe0c9c32a0f9a','{\"max_candidate_input_bytes\":1048576}','completed','2026-09-13T13:00:00Z','2026-09-13T13:05:00Z',0,1,0,0)",
            [],
        )?;
        for session_index in 0..session_count {
            for request_index in 0..requests_per_session {
                ...
                .enumerate()
                {
                    let applicable = *compressor == "json.minify"
                        ...
                        || (*compressor == "shell.diagnostic_projection" && request_index % 4 == 0);
                    ...
                    let _candidate = transaction.execute(
                        ...
                        params![candidate_id, snapshot_id, block_id, compressor, status, original, candidate, if applicable { Some(32_i64) } else { None }, if request_index == 0 { "high" } else { "medium" }, if compressor == &"json.readable_table" || compressor == &"json.compact_records" || compressor == &"json.key_elision" || compressor == &"json.empty_noise_fields" || compressor == &"json.repeated_value_elision" || compressor == &"shell.diagnostic_projection" || compressor == &"search.result_projection" { "human_readable_structured" } else if compressor == &"json.minify" || compressor == &"json.noop" { "provider_compatible_control" } else { "opaque_custom_encoding" }],
                    )?;
